...
local function __require(name)
	if not __cache[name] then
		if not __modules[name] then
			error("WindUI: module not found: " .. tostring(name))
		end
		__cache[name] = { result = __modules[name]() }
	end
	return __cache[name].result
end

__modules["AcrylicStub"] = function()
	return { init = function() end, Enable = function() end, Disable = function() end, AcrylicBlur = {}, AcrylicPaint = {} }
end
__modules["LocalizationStub"] = function()
...
__modules["Highlighter"] = function()
local highlighter = {}
local keywords = {
	lua = {
		"and",
...
local DynamicShapeModule = __require("DynamicShape")
...
function Creator.Init(WindUITable)
	WindUI = WindUITable
	Creator.ThemeFallbacks = __require("ThemeFallbacks")(Creator)
...
function Creator.AddLangObject() end
...
function Creator.SetLangForObject() end
function Creator:ChangeTranslationKey() end
function Creator.UpdateLang() end
function Creator.SetLanguage() end
...
__modules["ThemeFallbacks"] = function()
...
__modules["SvcInit"] = function()
return {
    platoboost = {
        Name = "Platoboost",
        Icon = "rbxassetid://75920162824531",
        Args = {"ServiceId", "Secret"},
        New = __require("SvcPlato").New
    },
    pandadevelopment = {
        Name = "Panda Development",
        Icon = "panda",
        Args = {"ServiceId"},
        New = __require("SvcPanda").New
    },
    luarmor = {
        Name = "Luarmor",
        Icon = "rbxassetid://130918283130165",
        Args = {"ScriptId", "Discord"},
        New = __require("SvcLuarmor").New
    },
    junkiedevelopment = {
        Name = "Junkie Development",
        Icon = "rbxassetid://106310347705078",
        Args = {"ServiceId", "ApiKey", "Provider"},
        New = __require("SvcJunkie").New
    },
}
end
...
__modules["PopupElement"] = function()
local Element = {}
...
function PopupModule.new(PopupConfig, Parent)
    ...
    local DialogInit = __require("WinDialog")
    ...
    local CreateButton = __require("UiButton").New
...
__modules["SearchIcons"] = function()
return {
...
__modules["UiCode"] = function()
local Code = {}
local Creator = __require("Creator")
local New = Creator.New
local Tween = Creator.Tween
local Highlighter = __require("Highlighter")
...
local CreateInput = __require("ElInput").New
local Creator = __require("Creator")
local New = Creator.New
local Tween = Creator.Tween
	...
	function DropdownModule:Refresh(Values)
		...
		for Index, Tab in next, Values do
			if Tab.Type ~= "Divider" then
				...
				__require("ElDivider"):New({ Parent = Dropdown.UIElements.Menu.Frame.ScrollingFrame })
			end
		end
...
__modules["UiScrollSlider"] = function()
local ScrollSlider = {}
...
local UserInputService = cloneref(game:GetService("UserInputService"))
local TagModule = __require("UiTag")
...
__modules["WinSection"] = function()
local Section = {}
local Creator = __require("Creator")
local New = Creator.New
local Tween = Creator.Tween
local TabModule = __require("WinTab")
...
local Creator = __require("Creator")
local New = Creator.New
local CreateToolTip = __require("UiTooltip").New
local CreateScrollSlider = __require("UiScrollSlider").New
...
function TabModule.New(Config, UIScale)
	...
	local ElementsModule = __require("ElsInit")
	ElementsModule.Load(
		Tab,
		Tab.UIElements.ContainerFrame,
...
__modules["WinOpenbutton"] = function()
local OpenButton = {}
...
function OpenButton.New(Window)
	...
	local IconButton = New("ImageButton", {
		...
		ZIndex = 99,
		Active = true,
		Image = Window.Icon or "rbxassetid://85262994427353",
		ScaleType = Enum.ScaleType.Fit,
	}, {
		UIScale,
		New("UICorner", { CornerRadius = UDim.new(1, 0) }),
		New("UIStroke", { Thickness = 0, ApplyStrokeMode = "Border" }),
	})

	OpenButtonMain.Button = IconButton

	Creator.AddSignal(IconButton:GetPropertyChangedSignal("AbsoluteSize"), function()
		Container.Size = UDim2.new(0, IconButton.AbsoluteSize.X, 0, IconButton.AbsoluteSize.Y)
	end)

	Creator.AddSignal(IconButton.MouseEnter, function()
		Tween(IconButton, 0.1, { BackgroundTransparency = 0.1 }):Play()
	end)
	Creator.AddSignal(IconButton.MouseLeave, function()
		Tween(IconButton, 0.1, { BackgroundTransparency = 0.25 }):Play()
	end)
	...
	function OpenButtonMain:Edit(Config)
		...
		if Config.OnlyMobile ~= false then
			Config.OnlyMobile = true
		else
			Window.IsPC = false
		end
		if Config.Draggable == false and DragModule then
			DragModule:Set(false)
		end
		if Config.Position and Container then
			Container.Position = Config.Position
		end
		if Config.Icon then
			OpenButtonMain:SetIcon(Config.Icon)
		end
		OpenButtonMain:SetScale(Config.Scale or 1)
	end
...
__modules["KeySystem"] = function()
local KeySystem = {}
local Creator = __require("Creator")
local New = Creator.New
local Tween = Creator.Tween
local CreateButton = __require("UiButton").New
local CreateInput = __require("UiInput").New
function KeySystem.new(Config, Filename, func, keyValidator)
	local KeyDialogInit = __require("WinDialog")
...
__modules["ElParagraph"] = function()
local Creator = __require("Creator")
local New = Creator.New
local Element = {}
local CreateButton = __require("UiButton").New
function Element:New(ElementConfig)
	...
	local Paragraph = __require("WinElement")(ElementConfig)
...
function Element:New(Config)
	...
	Button.ButtonFrame = __require("WinElement")({
		Title = Button.Title,
		Desc = Button.Desc,
		Parent = Config.Parent,
...
__modules["ElToggle"] = function()
local Creator = __require("Creator")
local New = Creator.New
local Tween = Creator.Tween
local CreateToggle = __require("UiToggle").New
local CreateCheckbox = __require("UiCheckbox").New
local Element = {}
function Element:New(Config)
	local Toggle = {
		__type = "Toggle",
	...
	Toggle.ToggleFrame = __require("WinElement")({
		Title = Toggle.Title,
		Desc = Toggle.Desc,
		Window = Config.Window,
		Parent = Config.Parent,
...
function Element:New(Config)
	...
	Slider.SliderFrame = __require("WinElement")({
		Title = Slider.Title,
		Desc = Slider.Desc,
		Parent = Config.Parent,
	...
	if Slider.IsTooltip then
		Tooltip = __require("UiTooltip").New(
			Value,
...
__modules["ElProgressBar"] = function()
local Creator = __require("Creator")
local New = Creator.New
local Tween = Creator.Tween
local Element = {}
...
function Element:New(Config)
	...
	ProgressBar.ProgressBarFrame = __require("WinElement")({
...
local CreateLabel = __require("UiLabel").New
function Element:New(Config)
	...
	local CanCallback = true
	Keybind.KeybindFrame = __require("WinElement")({
		Title = Keybind.Title,
		Desc = Keybind.Desc,
		Parent = Config.Parent,
		TextOffset = 85,
...
local function CreatePillUI(inputFrame, initialValue, callback)
	local New = Creator.New
	local items = {}
	local pillObjects = {}
	...
	local container = New("ScrollingFrame", {
		...
		BackgroundTransparency = 1,
		Parent = inputFrame.UIElements.Container,
	...
	local function fireCallback()
		callback(table.concat(items, ","))
	end

	local function buildPills()
		for _, p in ipairs(pillObjects) do
			p:Destroy()
		end
		pillObjects = {}
		for i, item in ipairs(items) do
			...
			if trimmed ~= "" then
				...
				local idx = i
				pill:FindFirstChildWhichIsA("TextButton", true).MouseButton1Click:Connect(function()
					table.remove(items, idx)
					buildPills()
					fireCallback()
				end)
				table.insert(pillObjects, pill)
			end
		end
	end

	local addInput = New("TextBox", {
		Size = UDim2.new(0, 80, 0, 26),
		BackgroundColor3 = Color3.fromHex("2a2a3a"),
		Text = "",
		PlaceholderText = "Add...",
		TextSize = 12,
		TextColor3 = Color3.new(1, 1, 1),
	...
	addInput.Parent = container

	addInput.FocusLost:Connect(function(enterPressed)
		if enterPressed then
			local v = addInput.Text:match("^%s*(.-)%s*$")
			if v ~= "" then
	...
	if initialValue and initialValue ~= "" then
		for part in (initialValue .. ","):gmatch("([^,]+),") do
			local t = part:match("^%s*(.-)%s*$")
			if t ~= "" then
				table.insert(items, t)
			end
	...
	return container, setVal, function()
...
function Element:New(Config)
	local Input = {
		...
		PillMode = Config.PillMode or false,
		UIElements = {},
		Width = 150,
	}
	...
	Input.InputFrame = __require("WinElement")({
		Title = Input.Title,
		Desc = Input.Desc,
		Parent = Config.Parent,
		TextOffset = Input.Type == "Textarea" and 0 or Input.Width,
		Hover = false,
		Tab = Config.Tab,
		Index = Config.Index,
		Window = Config.Window,
	...
	if Input.Type == "Textarea" and Input.PillMode then
		local pillContainer, pillSetVal, pillGetVal = CreatePillUI(
			Input.InputFrame,
			Input.Value,
			function(v)
				Input.Value = v
		...
		function Input:Set(v, isUserInput)
			if CanCallback then
				Input.Value = v or ""
				pillSetVal(Input.Value)
				Creator.SafeCallback(Input.Callback, Input.Value)
			end
		end
	...
	if Input.Type ~= "Textarea" then
		...
		InputComponent.Size = UDim2.new(1, 0, 0, 148)
	end
...
local CreateLabel = __require("UiLabel").New
local CreateInput = __require("UiInput").New
local CreateDropdown = __require("UiDropdown").New
local CurrentCamera = workspace.CurrentCamera
local Element = {
	UICorner = 10,
	UIPadding = 12,
...
function Element:New(Config)
	...
	Dropdown.DropdownFrame = __require("WinElement")({
...
__modules["ElCode"] = function()
local Creator = __require("Creator")
local New = Creator.New
local CodeComponent = __require("UiCode")
local Element = {}
function Element:New(Config)
	local Code = {
		__type = "Code",
...
__modules["ElsInit"] = function()
return {
	Elements = {
		Paragraph = __require("ElParagraph"),
		Button = __require("ElButton"),
		Toggle = __require("ElToggle"),
		Slider = __require("ElSlider"),
		ProgressBar = __require("ElProgressBar"),
		Keybind = __require("ElKeybind"),
		Input = __require("ElInput"),
		Dropdown = __require("ElDropdown"),
		Code = __require("ElCode"),
		Section = __require("ElSection"),
		Divider = __require("ElDivider"),
		Space = __require("ElSpace"),
		Image = __require("ElImage"),
		Group = __require("ElGroup"),
		HStack = __require("ElHStack"),
		VStack = __require("ElVStack"),
		Viewport = __require("ElViewport"),
	},
...
local CreateButton = __require("UiButton").New
local CreateScrollSlider = __require("UiScrollSlider").New
local Tag = __require("UiTag")
local ConfigManager = __require("ConfigInit")
...
return function(Config)
	...
	local TabModuleMain = __require("WinTab")
	local SectionModule = __require("ElSection")
	...
	local DialogModule = __require("WinDialog")
...
local WindUI = {
	Window = nil,
	Theme = nil,
	Creator = __require("Creator"),
	NotificationModule = __require("Notification"),
	Themes = nil,
	Transparent = false,
...
local Package = HttpService:JSONDecode(__require("PackageStub"))
if Package then
	WindUI.Version = Package.version
end
local KeySystem = __require("KeySystem")
...
function WindUI:ToggleAcrylic(Value) end
...
function WindUI:Popup(PopupConfig)
	...
	return __require("PopupInit").new(PopupConfig, WindUI.ScreenGui.Popups)
end
WindUI.Themes = __require("ThemesInit")(WindUI, Creator)
Creator.Themes = WindUI.Themes
WindUI:SetTheme("Dark")
function WindUI:CreateWindow(Config)
	local CreateWindow = __require("WinInit")
	if not RunService:IsStudio() and writefile then
